...
LCP measures when the largest visible content element renders. Usually this is a hero image, large text block, background image, or SVG element.
...
**5. Speculation Rules API for instant navigations**
```html
...
`eagerness` settings: `conservative` (on pointerdown), `moderate` (~200ms hover), `eager` (link in viewport), `immediate` (on page load). Start with `moderate`.
...
INP measures responsiveness across ALL interactions (clicks, taps, key presses). It reports the worst interaction (98th percentile for high-traffic pages).
...
// ✅ Break into chunks using scheduler.yield()
...
```javascript
// ✅ Prioritize visual feedback, then yield before heavy work
