...
A class of AI search engines (ChatGPT search, Perplexity, Gemini Overviews) cite web pages from their training and retrieval pipelines, not from the classic ranked results. A few things are low-cost and won't hurt:

- **Don't block AI crawlers wholesale.** `OAI-SearchBot`, `PerplexityBot`, `GoogleOther`, `Google-Extended`, `ClaudeBot` each have separate `robots.txt` user-agents. Decide per-bot rather than blanket-blocking.
- **Lean on schema.org `Article`/`Product`/`FAQPage`.** AI summarizers parse structured data more reliably than prose layouts.
- **Make first-paragraph answers self-contained.** Both featured snippets and AI summaries pull short, coherent passages. A definition or direct answer in the first 1-2 sentences is more extractable than content buried under marketing prose.

### `llms.txt` — emerging, unproven

[`llms.txt`](https://llmstxt.org/) is a proposed convention for LLMs to consume (a Markdown index of your site's important pages, served at `/llms.txt`). No major AI vendor has confirmed they read it. Treat it as a 5-minute speculative add for content sites — not a meaningful ranking factor.
